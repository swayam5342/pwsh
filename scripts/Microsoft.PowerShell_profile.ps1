...
Function quote {
    ...
    $url = if ($Type -eq "today" -or $Type -eq "author") {
        $baseUrl + $Type
    } else {
        $baseUrl + "random"
    }
    ...
    if ($Type -eq "author") {
        $quote = $response[0].q
        $author = $response[0].a
        Write-Output "Quote by ${author}: $quote"
    } else {
        $quote = $response[0].q
...
function ddow { Set-Location -Path $HOME\Downloads }
...
speed - To find out internet speed (Using )
...
bat - Enhanced 'cat' clone with syntax highlighting and more.

btop - Resource monitor that shows CPU, memory, and network usage.

croc - Securely transfers files across networks.
...
gping - Ping utility with a real-time graph.

hyperfine - Benchmarking tool.

micro - Modern terminal-based text editor.

neovim - Advanced version of Vim.
...
tokei - Provides statistics on code files.
